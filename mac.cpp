...
{	if(!sPassword.CStr()) return false;
	MD5_CTX md5; MD5_Init(&md5); unsigned char szMD5[16]; CString sMD5; sMD5.Assign("");
	MD5_Update(&md5, (unsigned char*)sPassword.Str(), sPassword.GetLength());
	MD5_Final(szMD5, &md5); for(int i=0;i<16;i++)
